...
def yamnet_frames_model_transfer(params, num_classes):
    waveform = layers.Input(batch_shape=(None,None), dtype=tf.float32)
    #waveform = layers.Input(batch_shape=(None,), dtype=tf.float32)
   # waveform_padded = features_lib.pad_waveform(waveform, params)
    #log_mel_spectrogram, features = features_lib.waveform_to_log_mel_spectrogram_patches(
     #   waveform_padded, params)
    embeddings = yamnet_transfer(waveform, params)
  #  prediction = embeddings
  #  last_layers = tf.keras.layers.Dense(256, activation='relu')
    last_layers = tf.keras.layers.Dense(num_classes, activation='softmax')
    #for layer in last_layers.layers[1:]:
    prediction = last_layers(embeddings)
    frames_model = Model(
        name='yamnet_frames', inputs=waveform,
        outputs=prediction)
    return frames_model
...
def yamnet_transfer_image(features, params):
   # net = layers.Reshape(
    #    (features.shape[0], features.shape[1], 3),
     #   input_shape=(params.patch_frames, params.patch_bands))(features)
    for (i, (layer_fun, kernel, stride, filters)) in enumerate(_YAMNET_LAYER_DEFS):
        features = layer_fun('layer{}'.format(i + 1), kernel, stride, filters, params)(features)
    embeddings = layers.GlobalAveragePooling2D()(features)
    return embeddings


def yamnet_frames_model_transfer_image(params, num_class):
    waveform = layers.Input(batch_shape=(None,None,None,3), dtype=tf.float32)
    #waveform = layers.Input(batch_shape=(None,), dtype=tf.float32)
   # waveform_padded = features_lib.pad_waveform(waveform, params)
    #log_mel_spectrogram, features = features_lib.waveform_to_log_mel_spectrogram_patches(
     #   waveform_padded, params)
    embeddings = yamnet_transfer_image(waveform, params)
  #  last_layers = tf.keras.layers.Dense(256, activation='relu')
    last_layers = tf.keras.layers.Dense( num_class, activation='softmax')
    #for layer in last_layers.layers[1:]:
    prediction = last_layers(embeddings)
    frames_model = Model(
        name='yamnet_frames', inputs=waveform,
